...
1.  **Future Annotations**: Always include `from __future__ import annotations` as the **very first executable line** of the file (immediately after the module docstring).
    *   *Correct*:
        ```python
        """Module docstring."""
        from __future__ import annotations
...
-   Do **not** use explicit title headings (e.g., `My Module\n=========`).
-   Use Sphinx cross-references (`:class:`, `:func:`, `:mod:`) to link to exported members.
-   Keep it concise; let the `README.rst` handle the broad overview.
...
## Repository Structure & Release Process

### Standard Structure
All `geometor` repositories should follow this layout:
-   `src/geometor/<package>/`: Source code (namespace package).
-   `src/geometor/<package>/__init__.py`: Contains `__version__`.
-   `CHANGELOG.rst`: RST-formatted changelog.
-   `README.rst`: Project overview and usage.
-   `pyproject.toml`: Build configuration and dependencies.

### Release Workflow
To release a new version:

1.  **Update Version**: Increment `__version__` in `src/geometor/<package>/__init__.py`.
2.  **Update Changelog**: Add a new section in `CHANGELOG.rst` with the version number and a list of changes.
3.  **Commit**: Commit these changes (e.g., `chore: bump version to x.y.z`).
4.  **Tag**: Create a git tag matching the version (e.g., `git tag v0.1.0`).
5.  **Push**: Push the commit and the tag (`git push origin main --tags`).